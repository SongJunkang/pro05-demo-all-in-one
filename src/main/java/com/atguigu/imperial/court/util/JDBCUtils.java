...
/**
 * 功能1。从数据源获取数据库连接
 * 功能2.从数据库获取到数据库链接后，绑定到本地线程，借助ThreadLocal
 * 功能3：释放线程时和本地线程解除绑定。
 *
 */
...
//
//
//    //数据源成员变量，设置为静态资源，保证大对象的单例性，同时保证静态方法可以访问。
//    private static DataSource dataSource;
//
//    // 由于 ThreadLocal 对象需要作为绑定数据时 k-v 对中的 key，所以要保证唯一性
//    // 加 static 声明为静态资源即可保证唯一性
...
//            //为了保证程序代码可移植性，需要基于一个确定的基准读取这个文件。
//            //确定的基准：类路径的根目录，resources目录下的内容经过构建操作中的打包操作后确定
//            // 放在WEB-INF/classes 目录存放编译好的 *class 字节码文件，所以这个目录，我们称之为类路径。
//            //类路径无论在本地运行，还是在服务器运行都是一个确定的基准。
//            //操作代码：
//            //1、获取当前类的类加载器。
//            ClassLoader classLoader = JDBCUtils.class.getClassLoader();
//
//            //2、通过类加载器对象，从类路径根目录下读取文件。
//            InputStream stream = classLoader.getResourceAsStream("jdbc.properties");
//            Properties properties = new Properties();
//
//            properties.load(stream);
//            //根据Properties对象，（已封装了数据库连接信息）来创建数据源对象。
//
//            dataSource = DruidDataSourceFactory.createDataSource(properties);
//
//        } catch (Exception e) {
//            e.printStackTrace();
//
//            //为了避免在真正抛出异常后，catch块捕捉到异常从而掩盖问题，
//            //这里将捕捉到的异常 封装为运行时异常 继续抛出。
//            throw new RuntimeException(e);
//        }
//    }
//    public static Connection getConnection(){
...
//
//        try {
//            //1、尝试从当前线程检查是否存在已经存在的Connection对象
//            connection = threadLocal.get();
//
//            if (connection == null){
//
